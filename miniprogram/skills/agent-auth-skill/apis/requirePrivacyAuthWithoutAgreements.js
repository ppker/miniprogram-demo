const { assertType, runAssertions } = require('../utils/assert.js')
const {
  isAgentPrivacyAuthorizeAvailable,
  callAgentPrivacyAuthorize
} = require('../utils/privacyAuthorize.js')

async function requirePrivacyAuthWithoutAgreements() {
  if (!isAgentPrivacyAuthorizeAvailable()) {
    return {
      isError: true,
      content: [{ type: 'text', text: '当前基础库不支持 wx.requestAgentPrivacyAuthorization，无法验证无协议授权卡片。请直接告知用户该能力暂不可用，禁止重试本接口。' }],
      structuredContent: {
        available: false,
        authorized: false,
        agreementCount: 0,
        errMsg: 'wx.requestAgentPrivacyAuthorization is not a function'
      }
    }
  }

  try {
    const res = await callAgentPrivacyAuthorize()
    const result = {
      available: true,
      authorized: !!res.authorized,
      agreementCount: 0,
      errMsg: res.errMsg || ''
    }
    const assertions = runAssertions([
      ['authorized 是 boolean', () => assertType(result.authorized, 'boolean')]
    ])

    return {
      isError: !assertions.passed,
      content: [{
        type: 'text',
        text: assertions.passed
          ? `无协议列表的隐私授权卡片已完成，卡片应仅展示描述文案，用户${result.authorized ? '同意' : '拒绝'}授权。请用一句简短话术告知用户结果。`
          : `无协议授权卡片断言失败：${assertions.failures.map(item => item.errMsg).join('；')}`
      }],
      structuredContent: Object.assign({}, result, { assertions })
    }
  } catch (err) {
    const errMsg = (err && (err.errMsg || err.message)) || String(err)
    console.error('[requirePrivacyAuthWithoutAgreements] error', err)
    return {
      isError: true,
      content: [{ type: 'text', text: `拉起无协议授权卡片失败：${errMsg}。请直接告知用户授权未完成，禁止重试本接口。` }],
      structuredContent: {
        available: true,
        authorized: false,
        agreementCount: 0,
        errMsg
      }
    }
  }
}

module.exports = requirePrivacyAuthWithoutAgreements
